        ...
        <div class="row table-responsive ">
            ...
            <div class="mt-5 col-lg-12">
                <div class="input-group mb-3">
                    <label class="input-group-text" for="selectsoursechange">اسم الطالب</label>
                    <select class="form-select" id="selectsoursechange" name="student_id" required>
...
$(document).ready(function () {
    ...
    $('#selectsoursechange').change(function (){
        ...
        callAjaxForCourse(student_id);
        $("#this_week").show();
        $("#next_week").show();
        $("#last_week").show();
    });

    function callAjaxForCourse(student_id) {
        $.ajax({
            url        :window.location.origin +'/admin/calendar_bystudent/'+student_id,
            type       :'GET',
            dataType   :'JSON',
            ...
            result.forEach(function(item){
                ...
                var i = 0 ;
                    days.forEach(function(currentValue, index)  {
                        if(new Date($("#"+currentValue[0]).html()).toLocaleDateString('en-CA') >= data_startdate && new Date($("#"+currentValue[0]).html()).toLocaleDateString('en-CA') <= data_enddate){
