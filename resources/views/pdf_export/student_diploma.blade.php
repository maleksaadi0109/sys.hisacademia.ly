...
<body>
    ...
    <table style="width: 100%;">
        <tbody>
            <tr>
                ...
                <td style="width: 85%;">{{$revenue[0]->user->name}}</td>
            <tr>
        </tbody>
        <tbody>
            <tr>
    ...
    <table style="width: 100%; text-align: center;">
        ...
        <tr>
            ...
            <td>اسم الكورس</td>
            <td>التوقيت</td>
            <td colspan="2">الايام</td>
        </tr>
        @foreach($revenue as $value)
        <tr>
            <td>{{$value->id}}</td>
            <td>{{$value->date_of_rec}}</td>
            ...
            <td colspan="1">من {{date("h:i A", strtotime($value->course->start_time))}} <br/> الى {{date("h:i A", strtotime($value->course->end_time))}}</td>
            <td colspan="2">
                <div>
                    @foreach(json_decode($value->course->days) as $value)
        ...
        <tr>
            ...
            <td>{{$revenue[0]->diploma->price}}</td>
            <td>{{0}}</td>
            <td>{{0}}</td>
            <td rowspan="2">{{$revenue[0]->diploma->price}}</td>
            <td rowspan="2">{{$revenue[0]->value_rec}}</td>
            <td rowspan="2">{{$revenue[0]->value_rem . ' ' . $revenue[0]->currency}}</td>
        </tr>
        <tr>
