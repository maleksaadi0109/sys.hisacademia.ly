...
@section('content')
    <section>
        <header>
            <h2 class="text-lg font-medium text-gray-900">{{__('استكمال المتبقي من الدبلوم')}}</h2>
        </header>

    <div class="container">
        <div class="row">
            <form method="POST" action="{{route('financial.update.diploma_revenue',['id' => $revenue[0]->diploma->id, $revenue[0]->user_id])}}">
                @csrf
                @method('patch')
                <div class="row">
                    <h5>سعر هذا الدبلوم (السعر: {{$revenue[0]->diploma->price}})</h5>
                    <h5>المدفوع من هذه الفاتورة:( {{$revenue[0]->value_rec}})</h5>
                    <h5>المتبقي من هذه الفاتورة:( {{$revenue[0]->value_rem}})</h5>
